...
def align(src_tokens, tgt_tokens, redis=False):
    """
...
def readSrcLine(src_line, src_dict, src_feature_dicts,
                _type="text", src_img_dir="", redis=False):
    srcFeats = None
    if _type == "text":
...
def readTgtLine(tgt_line, tgt_dict, tgt_feature_dicts, _type="text", redis=False):
    tgtFeats = None
    ...
    if tgt_feature_dicts:
        tgtFeats = [tgt_feature_dicts[j].
                    convertToIdx(tgtFeatures[j],
                                 onmt.Constants.UNK_WORD, redis=redis)
