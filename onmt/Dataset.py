...
if __name__ == "__main__":
    ...
    path = "/data/lixiaopu/corpus/cwmt17/0906/e2c/e2c.train.pt"
    dataset = torch.load(path, map_location=lambda storage, loc:storage)
    trainData = Dataset(dataset['train']['src'], dataset['train']['tgt'], 128, 3, srcFeatures=dataset['train'].get('src_features'), tgtFeatures=dataset['train'].get('tgt_features'), alignment=dataset['train'].get('alignments'))
    import time
    start = time.time()
    for idx in range(50):
        batch = trainData[idx]
